...
This is the **backend** repo. The frontend (Next.js 14 + TypeScript, `aws-amplify`
for Cognito auth) lives in the sibling `serverless-task-manager-frontend`
repo and is not provisioned by `template.yaml`.
...
└── .github/
    └── workflows/
        └── pipeline.yaml          # GitHub Actions CI/CD (sam pipeline init/bootstrap)
```
...
After deploy, copy these values into the frontend's `.env.local` (see the
`serverless-task-manager-frontend` repo) and the Amplify Console's
environment variables if you connect it there:
...
The frontend is the sibling **`serverless-task-manager-frontend`** repo
(Next.js 14 + TypeScript, `aws-amplify` for Cognito auth, Axios for API
calls) — not part of this repo, and not provisioned by `template.yaml`. Point
it at this stack's outputs (table above) via its own `.env.local`. Its
`GET /tasks` client expects a bare JSON array back (`GetTasksFunction`
returns `respond(200, tasks)`, not `{"tasks": [...]}`) — keep that contract
...
`.github/workflows/pipeline.yaml` is the GitHub Actions pipeline generated by
`sam pipeline bootstrap`/`sam pipeline init` — it builds, packages, and
deploys `template.yaml` using SAM CLI-managed, OIDC-federated pipeline
execution roles (no long-lived AWS keys in GitHub). This file is actively
maintained alongside the AWS-side bootstrap (pipeline execution roles,
CloudFormation execution roles, artifact buckets) — check it directly for
the current stages rather than relying on a stale summary here.
